...
class Paginate {
  ...
  renderPaginationList() {
    ...
    let paOne = `<li class="page-item "><a class="page-link" id='${this.currentPage - 1}' href="#">${
      this.currentPage - 1
    }</a></li>`;
    let paBegin = `<li class="page-item disabled"><a class="page-link" id='${this.currentPage}' href="#">${
      this.currentPage
    }</a></li>
     <li class="page-item "><a class="page-link" id='${this.currentPage + 1}' href="#">
     ${this.currentPage + 1}
     </a></li>
     <li class="page-item "><a class="page-link" id='${this.currentPage + 2}' href="#">${
      this.currentPage + 2
    }</a></li>`;

    if (this.currentPage >= this.totalPages) {
      this.pageNrColor = "disabled";
    }
    const paTwo = `<li class="page-item "><a class="page-link" id='${this.currentPage}' href="#">${this.currentPage}</a></li>`;
    let paThree = `<li class="page-item ${this.pageNrColor}"><a class="page-link" id='${
      this.currentPage + 1
    ...
    if (this.currentPage <= 1) {
      ...
      document.getElementById(
        "paginationPageView"
      ).innerHTML = `<ul class="pagination">${paOne}${paTwo}${paThree}</ul>`;
    }
  }

  getQueryData(pageNr, size) {
    axios
      .get(`/api/patientlist?pageNo=${pageNr}&size=${size}`)
      .then((response) => {
